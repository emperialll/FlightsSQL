...
QUERY_FLIGHT_BY_DATE = "SELECT flights.*, flights.ID as FLIGHT_ID, flights.ORIGIN_AIRPORT, flights.DESTINATION_AIRPORT, flights.DEPARTURE_DELAY as DELAY FROM flights WHERE flights.DAY = :day AND flights.MONTH = :month AND flights.YEAR = :year"
QUERY_FLIGHT_BY_AIRPORT = "SELECT flights.*, flights.ID as FLIGHT_ID, flights.ORIGIN_AIRPORT, flights.DESTINATION_AIRPORT, airlines.AIRLINE, flights.DEPARTURE_DELAY as DELAY FROM flights JOIN airlines ON flights.AIRLINE = airlines.ID WHERE flights.ORIGIN_AIRPORT = :origin_airport"
QUERY_FLIGHT_BY_AIRLINE = "SELECT airlines.*, airlines.ID as AIRLINE_ID, flights.ORIGIN_AIRPORT, flights.DESTINATION_AIRPORT, airlines.AIRLINE, flights.DEPARTURE_DELAY as DELAY FROM flights JOIN airlines ON  airlines.ID = flights.AIRLINE WHERE airlines.AIRLINE = :airline"
...
class FlightData:
    ...
    def _execute_query(self, query, params):
        ...
        with self._engine.connect() as conn:
          ...
          # Fetch all rows returned by the query and convert each row to a dictionary
